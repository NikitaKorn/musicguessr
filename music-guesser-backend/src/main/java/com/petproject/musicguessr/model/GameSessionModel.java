...
@Getter
@Setter
public class GameSessionModel<T extends AbstractSessionRoomHandler>{
    ...
    private GameSessionModel(Builder<T> builder) {
    ...
    public static class Builder<T extends AbstractSessionRoomHandler> {
        ...
        public GameSessionModel<T> build() {
            if (room == null) {
                throw new IllegalStateException("Room and path are required");
            }
            return new GameSessionModel<>(this);
        }
    }
}
